...
def ShashlikFaces(object, c1, event, util):
    ...
    if not util.stealthmode:
        ...
        gStyle.SetOptStat(0)
        c1.cd(1)
        #zax = hMapFront.GetZaxis()
        #hMapFront.SetAxisRange(hMapFront.GetMinimum(),1.2*zax.GetBinCenter(zax.GetNbins()),"Z")
        hMapFront.SetAxisRange(hMapFront.GetMinimum(),min(210,1.3*hMapFront.GetMaximum()),"Z")
        hMapFront.Draw('COLZ text')
        c1.cd(2)
        #zax = hMapBack.GetZaxis()
        #hMapBack.SetAxisRange(hMapBack.GetMinimum(),1.2*zax.GetBinCenter(zax.GetNbins()),"Z")
        #print 'setups to',hMapFront.GetMaximum(200),hMapBack.GetMaximum(200),hMapFront.GetMaximum(),hMapBack.GetMaximum()
        hMapBack.SetAxisRange(hMapBack.GetMinimum(),min(160,1.3*hMapBack.GetMaximum()),"Z")
